...
protected:
    ...
    panod::FeaturesMatcher::operator()_ work.
...
/** @brief Features matcher similar to cv::panod::BestOf2NearestMatcher which
finds two best matches for each feature and leaves the best one only if the
ratio between descriptor distances is greater than the threshold match_conf.

Unlike cv::panod::BestOf2NearestMatcher this matcher uses affine
transformation (affine trasformation estimate will be placed in matches_info).

@sa cv::panod::FeaturesMatcher cv::panod::BestOf2NearestMatcher
 */
