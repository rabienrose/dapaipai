...
void StitchAlgo::FinalImg(){
    ...
    vector<detail::ImageFeatures> features(num_images);
    ...
    for (size_t i = 0; i < cameras.size(); ++i){
        ...
        cameras[i].R = R;
        std::cout<<"Initial camera intrinsics #" << indices[i]+1 << ":\nK:\n" << cameras[i].K() << "\nR:\n" << cameras[i].R<<std::endl;
    }
...
//    }
    ...
    for (int img_idx = 0; img_idx < num_images; ++img_idx){
        ...
        if (!is_compose_scale_set)
        {
            ...
            {
                // Update intrinsics
                std::cout<<cameras[i].focal<<std::endl;
        ...
        if (!blender)
        {
            ...
            std::cout<<final_rect.tl().x<<" : "<<final_rect.tl().y<<std::endl;
            std::cout<<final_rect.br().x<<" : "<<final_rect.br().y<<std::endl;
...
//            }
    ...
    cv::Mat all_sphere_img=cv::Mat::zeros(warped_image_scale*2*3.1415926, warped_image_scale*3.1415926, CV_8UC4);
    all_sphere_img.colRange(final_rect.tl().y, final_rect.br().y).rowRange(final_rect.tl().x+1800, final_rect.br().x+1800)=result;
    cv::resize(result, sphere_img, cv::Size(3600,1800));
