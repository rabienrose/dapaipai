...
tracking_module::tracking_module(const std::shared_ptr<config>& cfg, system* system, data::map_database* map_db,
    ...
    : cfg_(cfg), camera_(cfg->camera_), system_(system), map_db_(map_db), bow_vocab_(bow_vocab), bow_db_(bow_db),
      ...
      keyfrm_inserter_(cfg_->camera_->setup_type_, cfg_->true_depth_thr_, map_db, bow_db, 0, 10000) {
    ...
    if (camera_->setup_type_ == camera::setup_type_t::Monocular) {
        ...
        ini_extractor_left_->set_max_num_keypoints(ini_extractor_left_->get_max_num_keypoints() * 4);
    }
...
//            std::cout<<"mp: "<<m_in_curr->get_pos_in_world().transpose()<<std::endl;
//        }
        // Tracking mode
...
bool tracking_module::new_keyframe_is_needed() const {
    ...
    bool re = keyfrm_inserter_.new_keyframe_is_needed(curr_frm_, num_tracked_lms_, *ref_keyfrm_);
