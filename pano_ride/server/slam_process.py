...
traj_name="20200815-yue-4square_slam"
raw_traj_name="20200815-yue-4square"
traj_info={"task":"gps","status":2,"name":"20200815-yue-4square_slam","author":"yue","date":20200815,"imgs":[]}
meta_traj = read_json_from_oss(oss_shangrila_raw+"/"+raw_traj_name+"/meta.json", "slam_", bucket)
gps_axis = meta_traj[int(len(meta_traj)/2)]['gps']
...
for i in range(len(pose_list)):
    x=(pose_list[i][2,3]-slam_axis[0])*scale+axis_utm[2]
    y=(pose_list[i][0,3]-slam_axis[1])*scale+axis_utm[3]
...
traj_table.update_one({"name":"20200815-yue-4square_slam"},{"$set":traj_info},True)
